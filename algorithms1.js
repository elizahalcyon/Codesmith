...
// http://csbin.io/cs-prep-algorithms/1
...
function mode(array) {
    ...
    // looking for largest value in object
    let currentKey = -Infinity;
      let largestFrequency = -Infinity
    
    for(let key in resultObj){
      let frequency = resultObj[key]
      let numKey = +key							
      
      if(frequency > largestFrequency){		//if current count is greater than largest 																									count
        largestFrequency = frequency;			//current count is largest
        currentKey = numKey;							//current Key is largest
      } 
    }
